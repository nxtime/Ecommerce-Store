    ...
    var productsContainer = new Container({
      ...
      children: Cart.state.items.map(function (_a) {
        ...
        var productButton = new Button({
          text: product.cart.toString(),
          onClick: function () {
            if (product.quantity === 0)
              return;
            Cart.add(product, callback);
            productButton.updateState(product.cart.toString());
          }
        });
        var cartProductItem = new Container({
          as: 'li',
          variant: {
            type: 'flex',
            gap: '2',
        ...
        var removeProductButton = new Button({
          ...
          onClick: function () {
            ...
            product.cart = 0;
            callback(product);
            Cart.state.items = Cart.state.items.filter(function (item) { return item.product.id !== product.id; });
            Cart.updateState();
